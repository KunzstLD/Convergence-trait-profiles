...
# load hc_output
hc_output_omit <- readRDS(file.path(data_cache,
                                    "hc_output_omit.rds"))
...
ls_dist_mat <- lapply(hc_output_omit, function(y) y[["distance_matrix"]])
...
for(i in names(ls_dist_mat)) {
  ...
  label <- hc_taxa %>% as.dendrogram() %>% labels()

  # extract labels and merge to data 
  data <- trait_subsets[[i]]
  
  dend_label <-
    merge(data.frame(label = label),
          data[, c("species", "ID_AQEM")],
          by.x = "label",
          by.y = "ID_AQEM")
  
  # save output
  dend_output[[i]] <- list("dend_label" = dend_label, 
                           "hc_element" = hc_taxa)
}
...
dend_output[["bsk"]]$hc_element %>%
  as.dendrogram() %>%
  color_branches(k = hc_output[["bsk"]]$optimal_nog) %>%
  hang.dendrogram(hang_height = 0.005) %>%
  set("labels_cex", 0.75) %>%
  dendextend::ladderize() %>%
  set("labels", dend_output[["bsk"]]$dend_label$species) %>%
  plot(horiz = TRUE)
dev.off()
